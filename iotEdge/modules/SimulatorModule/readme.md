...
This document shows how to stream USB camera feed over RTSP for host devices running Unix. 
...
The default pipeline encodes the USB camera stream in H264 format. 
...
## Video Integration
To use the USB camera stream, first make sure that the Docker container is accessible (i.e., your IoT Edge device has access to the container). Then, in your deployment manifest file, update the following:
