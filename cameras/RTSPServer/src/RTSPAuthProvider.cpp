...
public:
    ...
    STDMETHODIMP Authorize(LPCWSTR authResp, LPCWSTR authSesMsg, LPCWSTR mthd) override try
    {
        ...
        /* 
            NOTE: Temporary fix due to problem with Credential Store when rurnning as a Scheduled Task
            WARNING: For production deployments this MUST be fixed
        */
        if (!username.empty()){


            if (username != "eflow") {
                winrt::check_win32(ERROR_USER_PROFILE_LOAD);
            }
        ...
        else if (paramMapResponse["Authorization: "] == "Digest")
        {
            CryptographicHash hash(nullptr);
            if ((paramMapResponse["algorithm="] == "MD5") || (paramMapResponse["response="].size() < 64))
            {
